...
def main():
    parser = argparse.ArgumentParser(
        description="Extract minimal verifiable slice for a single function"
    )
    parser.add_argument('--project-dir', required=True,
                        help="Path to the project source directory")
    parser.add_argument('--func', default=None,
                        help="Target function name to slice")
    parser.add_argument('--out', default=None,
                        help="Output file path (default: stdout)")
    parser.add_argument('--mode', choices=['inline', 'stub'], default='inline',
                        help="How to handle callees: inline their bodies or just declarations")
    parser.add_argument('--list-functions', action='store_true',
                        help="Just list all functions found in the project")
    parser.add_argument('--list-layers', action='store_true',
                        help="Output bottom-up DAG layer ordering for verification")

    args = parser.parse_args()

    if args.list_functions:
        all_contents, all_functions, _, _ = build_project_index(args.project_dir)
        print(f"Found {len(all_functions)} functions:")
        for name, info in sorted(all_functions.items()):
            lines = info['end'] - info['sig_start'] + 1
            print(f"  {name:40s} {info['file']}:{info['sig_start']+1}-{info['end']+1} ({lines} lines)")
        return

    if args.list_layers:
        all_contents, all_functions, _, _ = build_project_index(args.project_dir)
        layers, call_graph = compute_layers(all_functions)
        max_layer = max(layers.values()) if layers else 0
        total = len(all_functions)
        print(f"# Bottom-up verification layers ({total} functions, {max_layer + 1} layers)")
        print(f"# Layer 0 = leaf functions (no project-internal callees)")
        print(f"# Layer N = all callees in layers < N")
        print()
        for layer_num in range(max_layer + 1):
            funcs_in_layer = sorted([f for f, l in layers.items() if l == layer_num])
            print(f"## Layer {layer_num} ({len(funcs_in_layer)} functions)")
            for fname in funcs_in_layer:
                callees = call_graph.get(fname, [])
                if callees:
                    print(f"  {fname} -> [{', '.join(sorted(callees))}]")
                else:
                    print(f"  {fname}")
            print()
        return
    ...
    if not args.func:
        parser.error("--func is required when not using --list-functions or --list-layers")

    result = generate_slice(args.func, args.project_dir, args.mode)
    if result is None:
        sys.exit(1)
    ...
    if args.out:
        ...
        with open(args.out, 'w') as f:
            f.write(result)
        print(f"Written: {args.out} ({result.count(chr(10))+1} lines)", file=sys.stderr)
    else:
        print(result)
